                ...
                <form onsubmit="window.saveNewRecord(event)" class="p-4 sm:p-5 space-y-5">
                    <div class="space-y-1">
                        ...
                        <div class="relative">
                            ...
                            <div id="clientDropdownModal" class="hidden absolute z-[110] left-0 right-0 mt-2 bg-dark-900 border border-white/10 rounded-2xl shadow-2xl max-h-48 overflow-y-auto custom-scroll p-2"></div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="space-y-1">
                            ...
                            <input type="date" name="date" required value="${r.date || r.data}" class="w-full bg-dark-900 border border-white/5 p-3 rounded-xl outline-none focus:border-amber-500/50 transition-all font-bold text-sm">
                        </div>
                        <div class="space-y-1">
                            <label class="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Horário</label>
                            <input type="time" name="time" required value="${r.time || r.horario}" class="w-full bg-dark-900 border border-white/5 p-3 rounded-xl outline-none focus:border-amber-500/50 transition-all font-bold text-sm">
                        </div>
                    </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <div class="relative">
                            ...
                            <div id="procedureDropdownModal" class="hidden absolute z-[110] left-0 right-0 mt-2 bg-dark-900 border border-white/10 rounded-2xl shadow-2xl max-h-48 overflow-y-auto custom-scroll p-2"></div>
                        </div>
                    </div>

                    <div class="space-y-1">
                        <label class="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Valor (R$)</label>
                        <input type="number" step="0.01" name="value" value="${r.value || r.valor || ""}" class="w-full bg-dark-900 border border-white/5 p-3 rounded-xl outline-none focus:border-amber-500/50 transition-all font-bold text-sm">
                    </div>

                    <div class="space-y-1">
                        <label class="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Pagamento</label>
                        <select name="payment" required class="w-full bg-dark-900 border border-white/5 p-3 rounded-xl outline-none focus:border-amber-500/50 transition-all font-bold text-sm appearance-none">
                            ${[
                              "PIX",
                              "DINHEIRO",
                              "CARTÃO",
