  ...
  return `
        <div class="px-4 pt-6 sm:px-8 sm:pt-6 space-y-6 animate-in fade-in duration-500 pb-32">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 text-sm">
                <div>
                    <h2 class="text-3xl font-display font-black">Saídas <span class="text-rose-500">${periodFilter === "total" ? "Totais" : periodFilter === "diario" ? `${state.filters.day} de ${monthsLong[targetMonth - 1]}` : periodFilter === "semanal" ? "da Semana" : `${monthsLong[targetMonth - 1]}${targetYear !== new Date().getFullYear() ? " " + targetYear : ""}`}</span></h2>
            ...
            <div class="flex flex-wrap gap-4 items-center bg-dark-900/50 p-4 rounded-[1.5rem] border border-white/5 shadow-2xl">
                ...
                <div class="flex gap-2 flex-wrap sm:flex-nowrap">
                    <select onchange="window.setExpenseFilter('expenseStatusFilter', this.value)" class="bg-dark-950 border border-white/10 px-4 py-3 rounded-xl outline-none focus:border-rose-500/50 transition-all font-bold text-xs uppercase text-white cursor-pointer min-w-[160px]">
                    ...
                    <select onchange="window.setExpenseFilter('expenseSort', this.value)" class="bg-dark-950 border border-white/10 px-4 py-3 rounded-xl outline-none focus:border-rose-500/50 transition-all font-bold text-xs uppercase text-white cursor-pointer min-w-[170px]">
            ...
            <div class="space-y-4 md:space-y-0 md:bg-dark-900/30 md:rounded-[2rem] border border-white/5">
                ...
                <div class="divide-y divide-white/5">
                      ...
                      filteredExpenses.length === 0
                        ...
                        <div class="flex flex-col md:grid md:grid-cols-[120px_130px_1fr_100px_110px_120px_80px] items-center px-6 py-2.5 hover:bg-white/[0.02] transition-colors group relative border-b border-white/5">
                            ...
                            <div class="w-full md:w-auto px-2 mt-2 md:mt-0 relative min-w-0">
                                ...
                                <div class="flex flex-col gap-0.5">
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <div contenteditable="true" data-id="${e.id}" data-field="cartao" onfocus="window.selectAll(this)" onblur="window.saveExpenseInline(this)" onkeydown="window.handleInlineKey(event)" oninput="window.showExpenseAutocomplete(this)" class="text-[10px] font-black text-amber-500 uppercase tracking-tight outline-none focus:bg-white/5 hover:bg-white/5 px-1 rounded transition-all truncate cursor-text">
                                            ${e.cartao || "OUTROS"}
                                        </div>
                                    </div>
                                    ${(() => {
                            ...
                            <div class="text-center mt-2 md:mt-0 px-2">
                                ...
                                <select onchange="window.toggleExpenseStatus('${e.id}', this.value)" class="bg-white/5 border border-white/10 text-[10px] font-black uppercase rounded-lg px-2 py-1 outline-none transition-all w-full ${e.paga ? "text-emerald-500" : diffDays < 0 ? "text-rose-500" : "text-amber-500"}">
                            ...
                            <div class="flex justify-center gap-2 mt-2 md:mt-0">
                                <button onclick="window.openExpenseModal(${JSON.stringify(e).replace(/"/g, "&quot;")})" class="w-8 h-8 rounded-full bg-amber-500/10 text-amber-500 hover:bg-amber-500 hover:text-dark-950 transition-all flex items-center justify-center"><i class="fas fa-edit text-[10px]"></i></button>
                                <button onclick="window.deleteExpense(${e.id})" class="w-8 h-8 rounded-full bg-rose-500/10 text-rose-500 hover:bg-rose-500 hover:text-white transition-all flex items-center justify-center"><i class="fas fa-trash text-[10px]"></i></button>
                            </div>
                        </div>
                      `;
                            })
                    ...
                    ${filteredExpenses.length > 0 ? `<div class="bg-white/[0.01] px-8 py-6 flex justify-between items-center border-t border-white/5"><span class="text-xs font-black uppercase tracking-widest text-slate-500">Total do Período</span><span class="text-xl font-black text-white">R$ ${totalGeral.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</span></div>` : ""}
                </div>
            </div>
                        ...
                        <form onsubmit="window.saveExpense(event)" id="expenseModal" class="p-5 space-y-4">
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div class="space-y-1">
                                    ...
                                    <input type="date" name="data_compra" value="${state.editingExpense?.data_compra || new Date().toISOString().split("T")[0]}" style="color-scheme: dark" class="w-full bg-dark-950 border border-white/5 p-3 rounded-xl outline-none focus:border-rose-500/50 transition-all font-bold text-xs">
                                </div>
                            </div>
                            <div class="space-y-1 relative">
                                <label class="text-[9px] font-black uppercase text-slate-500 tracking-widest ml-1">Descrição</label>
                                <input type="text" name="descricao" required id="expenseModalDesc" value="${state.editingExpense?.descricao || ""}" placeholder="EX: COMPRA 1, ALUGUEL..." autocomplete="off" oninput="window.showExpenseAutocomplete(this, true, 'desc')" onkeydown="window.handleEnterSelection(event, 'expenseAutocomplete_desc_modal')" onblur="setTimeout(() => document.getElementById('expenseAutocomplete_desc_modal')?.classList.add('hidden'), 200)" class="w-full bg-dark-950 border border-white/5 p-3.5 rounded-xl outline-none focus:border-rose-500/50 transition-all font-bold uppercase text-sm">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <div class="space-y-1">
                                    ...
                                    <input type="number" step="0.01" name="valor_total" value="${state.editingExpense?.valor_total || state.editingExpense?.valor || ""}" class="w-full bg-dark-950 border border-white/5 p-3 rounded-xl outline-none focus:border-rose-500/50 transition-all font-bold text-sm">
                                </div>
                                <div class="space-y-1">
                                    <label class="text-[9px] font-black uppercase text-slate-500 tracking-widest ml-1">Parcela</label>
                                    <input type="text" name="parcela" value="${state.editingExpense?.parcela || "1/1"}" oninput="window.maskParcela(this)" placeholder="1/1" class="w-full bg-dark-950 border border-white/5 p-3 rounded-xl outline-none focus:border-rose-500/50 transition-all font-bold text-xs text-center">
                                </div>
                                <div class="space-y-1">
                                    <label class="text-[9px] font-black uppercase text-slate-500 tracking-widest ml-1">Vencimento</label>
                                    <input type="date" name="vencimento" required value="${state.editingExpense?.vencimento || ""}" style="color-scheme: dark" class="w-full bg-dark-950 border border-white/5 p-3 rounded-xl outline-none focus:border-rose-500/50 transition-all font-bold text-xs">
                                </div>
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <div class="space-y-1">
                                    <label class="text-[9px] font-black uppercase text-slate-500 tracking-widest ml-1">Valor Parcela (R$)</label>
                                    <input type="number" step="0.01" name="valor" value="${state.editingExpense?.valor || ""}" class="w-full bg-dark-950 border border-white/5 p-3 rounded-xl outline-none focus:border-rose-500/50 transition-all font-bold text-sm">
                                </div>
                                <div class="space-y-1">
                                    <label class="text-[9px] font-black uppercase text-slate-500 tracking-widest ml-1">Valor Pago (R$)</label>
                                    <input type="number" step="0.01" name="valor_pago" value="${state.editingExpense?.valor_pago || ""}" class="w-full bg-dark-950 border border-white/5 p-3 rounded-xl outline-none focus:border-rose-500/50 transition-all font-bold text-sm text-emerald-500">
                                </div>
                            </div>
                            ...
                            <button type="submit" class="w-full bg-rose-500 text-white font-black py-4 rounded-xl border border-transparent shadow-lg shadow-rose-500/20 active:scale-95 uppercase tracking-widest text-xs transition-all mt-2">${state.editingExpense?.id ? "Salvar Alterações" : "Salvar Conta"}</button>
                        </form>
                    </div>
                </div>
            `
