            ...
            <div class="space-y-4">
                ...
                <div class="hidden md:grid grid-cols-12 gap-4 px-8 py-3 text-[9px] font-black text-slate-500 uppercase tracking-widest border-b border-white/5 bg-white/[0.01]">
                    <div class="col-span-2 pl-2 text-left cursor-pointer hover:text-amber-500 transition-colors flex items-center gap-2" onclick="window.togglePlanSort('nome')">
                        Cliente
                        <i class="fas fa-sort-${state.planSort === "nome_asc" ? "alpha-down" : "alpha-up"} text-amber-500 text-xs"></i>
                    </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-center p-6 hover:bg-white/[0.02] transition-colors group plan-client-card" data-name="${c.nome}">
                                    ...
                                    <div class="md:col-span-2 flex flex-col justify-center gap-1">
                                        <div class="flex items-center gap-1.5 text-[11px] font-black tracking-tighter group/limit">
                                            <span class="${planStats?.usageCount >= (c.limite_cortes || 99) ? "text-rose-500" : "text-emerald-500"}">${planStats?.usageCount || 0}</span>
                                        ...
                                        <div class="w-full h-1 bg-white/5 rounded-full overflow-hidden mt-1">
                                            <div class="h-full ${planStats?.usageCount >= (c.limite_cortes || 99) ? "bg-rose-500" : "bg-emerald-500"} transition-all duration-500" 
                                    ...
                                    <div class="md:col-span-2 flex flex-col justify-start">
                                        <input type="date" value="${c.plano_pagamento || ""}" 
                                               ...
                                               class="w-full bg-dark-900 border ${c.plano_pagamento && (new Date() - new Date(c.plano_pagamento)) / (1000 * 60 * 60 * 24) > 30 ? "border-rose-500/50" : "border-white/5"} text-[10px] font-bold rounded-xl px-2 py-2 outline-none focus:border-amber-500/50 transition-all text-white cursor-pointer hover:bg-white/5 text-left">
                                        ${c.plano_pagamento && (new Date() - new Date(c.plano_pagamento)) / (1000 * 60 * 60 * 24) > 30 ? `<span class="text-[8px] text-rose-500 font-bold mt-1 uppercase tracking-tighter ml-1">Vencido</span>` : ""}
                                    </div>
                    ...
                    <div class="bg-dark-900 border border-white/10 rounded-[2rem] w-full max-w-md p-6 shadow-2xl relative animate-in zoom-in-95 duration-200">
                        ...
                        <form id="plan-modal-form" onsubmit="window.saveNewPlanClient(event)" class="space-y-4" autocomplete="off">
                            <div class="space-y-1 relative">
                                ...
                                <div id="plan-modal-suggestions" class="hidden absolute left-0 right-0 top-full mt-2 bg-dark-900 border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200"></div>
                            </div>
                            <div class="space-y-1">
                                <label class="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">Plano</label>
                                <select name="plano" required class="w-full bg-dark-950 border border-white/5 p-3 rounded-xl outline-none focus:border-amber-500/50 transition-all font-bold text-white cursor-pointer appearance-none">
                            ...
                            <div class="space-y-1">
                                ...
                                <div class="relative">
                                    <input type="number" name="limite_cortes" value="99" min="1" max="99" class="w-full bg-dark-950 border border-white/5 p-3 rounded-xl outline-none focus:border-amber-500/50 transition-all font-bold text-white pl-10">
                                    <i class="fas fa-scissors absolute left-4 top-1/2 -translate-y-1/2 text-slate-600 text-xs"></i>
                                </div>
                            </div>
                            <div class="pt-4">
                                <button type="submit" class="w-full bg-amber-500 text-dark-950 font-black py-4 rounded-xl hover:shadow-lg hover:shadow-amber-500/20 transition-all uppercase tracking-widest text-xs">Cadastrar Assinante</button>
                            </div>
                        </form>
                    </div>
                </div>
